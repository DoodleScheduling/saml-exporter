...
KUSTOMIZE := kustomize
IMG := ghcr.io/doodlescheduling/saml-exporter:latest
...
build:
	...
	CGO_ENABLED=0 go build -o saml-exporter main.go
	cp grafana/dashboard.json chart/saml-exporter
...
kind-test: ## Deploy including test
	...
	kubectl --context kind-${CLUSTER} -n saml-system delete pods --all
	kustomize build config/tests/cases/${TEST_PROFILE} --enable-helm | kubectl --context kind-${CLUSTER} apply -f -	
	kubectl --context kind-${CLUSTER} -n saml-system wait --for=condition=Ready pods -l app.kubernetes.io/managed-by!=Helm -l verify=yes --timeout=3m
	kubectl --context kind-${CLUSTER} -n saml-system wait --for=jsonpath='{.status.conditions[1].reason}'=PodCompleted pods -l app.kubernetes.io/managed-by!=Helm -l verify=yes --timeout=3m
	kustomize build config/tests/cases/${TEST_PROFILE} --enable-helm | kubectl --context kind-${CLUSTER} delete -f -	

.PHONY: deploy
deploy:
	cd deploy/exporter && $(KUSTOMIZE) edit set image ghcr.io/doodlescheduling/saml-exporter=${IMG}
